...
public class InvestInternalEtfApi {
    ...
    @Operation(
            summary = "ETF 자동 투자 체결 이력 조회 - 내부 API",
            description = "내부 서비스가 X-Service-ID, X-Internal-Api-Key, X-User-UUID 헤더로 호출하는 ETF 자동 투자 체결 이력 조회 API입니다."
    )
    @SecurityRequirement(name = "SERVICE_ID")
    @SecurityRequirement(name = "INTERNAL_API_KEY")
    @GetMapping("/internal/invest/accounts/{accountUuid}/auto-invest/executions")
    public ResponseEntity<ApiResponse<InvestAutoInvestExecutionHistoryResponse>> getInternalAutoInvestExecutionHistories(
            @AuthenticationPrincipal AuthenticatedUser authenticatedUser,
            @Parameter(description = "조회할 증권계좌 UUID", required = true)
            @PathVariable UUID accountUuid,
